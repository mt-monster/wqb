...
Alphas must pass a series of pre-submission checks (viewable via `mcp__wq-brain-http__get_alpha_details` → `is.checks`) to ensure they meet quality thresholds. Key tests include:
...
- **Improve Metrics**: Apply `ts_decay_linear` for stability, `scale` for normalization, and check with `mcp__wq-brain-http__check_correlation`.
- **Common Pitfalls**: Avoid high correlation (use `mcp__wq-brain-http__check_correlation`), ensure non-NaN data (e.g., via `ts_backfill`), and target high IR/Fitness.
...
A powerful way to source fresh ideas is through academic papers on arXiv. Use the `arxiv_api.py` script shipped in the `wq-brain-alpha-optimization-v1` skill (`scripts/arxiv_api.py`, detailed in its `arXiv_API_Tool_Manual.md`) to search and download relevant research.

- **Search Example**: Run `python wq-brain-alpha-optimization-v1/scripts/arxiv_api.py "quantitative finance momentum strategies"` to find papers on momentum ideas. Download top results for detailed study.
...
5. **Central Tendency**: For example, Simulate `ts_mean(datafield, 252) > X` (vary X). Insight: Typical values over ~1 year. Never use `ts_median` (ghost operator; CANCELS the whole multi-sim).
...
- Use tools like `mcp__wq-brain-http__workflow_submit_alpha` for pre-checks and submission.
