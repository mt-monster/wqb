...
- **`mcp__wq-brain-http__get_datasets`**: Discover available datasets with coverage filters
- **`mcp__wq-brain-http__get_datafields`**: Get field count and coverage statistics for selected dataset
- **`mcp__wq-brain-http__get_documentations`**: Access platform documentation and research papers
- **`mcp__wq-brain-http__get_documentation_page`**: Read specific documentation pages for dataset context
...
- **`mcp__wq-brain-http__get_datafields`**: Retrieve complete field inventory with metadata
- **`mcp__wq-brain-http__get_documentation_page`**: Read data type handling documentation (e.g., "vector-datafields", "group-data-fields")
- **`mcp__wq-brain-http__get_operators`**: Understand available operators for different data types
- **`mcp__wq-brain-http__get_documentations`**: Access data handling best practices and examples
...
1. **Statistical Profiling Using BRAIN 6-Tips Methodology**: Run systematic exploratory simulations following the proven BRAIN platform approach. This methodology provides a comprehensive framework for understanding new datafields efficiently. **Critical Settings for All Tests**:
   ...
   **E. Central Tendency Analysis**
   - **Expression**: `ts_mean(datafield, 252) > X` (for matrix) or `ts_mean(vector_operator(datafield), 252) > X` (for vector)
   - **Purpose**: Understand typical values and central tendency
   - **Insight**: Mean of the datafield over ~1 year
   - **Watch Out**: Do **not** use `ts_median` — ghost operator; it CANCELS the whole multi-sim batch
   - **Testing Strategy**: Vary X values to understand value distribution:
     - **X = 0**: Test if mean is positive
   ...
   **Test E (Tendency)**: `ts_mean(close,252) > 0` → Should show high counts (mean prices are positive)
...
- **`create_multi_simulation`**: Execute BRAIN 6-tips methodology simulations
- **`mcp__wq-brain-http__get_platform_setting_options`**: Validate simulation settings and parameters
- **`mcp__wq-brain-http__get_operators`**: Access time series operators (ts_std_dev, ts_mean, scale_down). Never use ghost `ts_median`.
- **`mcp__wq-brain-http__get_documentation_page`**: Read simulation settings documentation ("simulation-settings")
- **`mcp__wq-brain-http__get_documentation_page`**: Access data analysis best practices ("data")
...
- **`mcp__wq-brain-http__get_documentation_page`**: Access field description best practices ("data")
- **`mcp__wq-brain-http__get_documentations`**: Review documentation structure and organization
- **`get_documentation_page`**: Find usage examples in documentation ("19-alpha-examples")
- **`mcp__wq-brain-http__get_documentation_page`**: Access categorization guidelines ("how-use-data-explorer")
...
- **`search_forum_posts`**: Search community discussions (fallback: `search_forum_posts`)
- **`read_forum_post`** / **`mcp__wq-brain-http__read_forum_post`**: Read detailed forum discussions and best practices
- **`mcp__wq-brain-http__get_events`**: Access competition guidelines and rules
- **`mcp__wq-brain-http__get_competition_details`**: Review specific competition requirements
- **`mcp__wq-brain-http__get_documentation_page`**: Access platform best practices and guidelines
- **`get_documentation_page`**: Review alpha strategy examples for validation
...
- **Data Explorer**: Use `mcp__wq-brain-http__get_datasets` and `mcp__wq-brain-http__get_datafields`
- **Simulation Engine**: Use `mcp__wq-brain-http__create_simulation` with proper settings
- **Documentation System**: Use `mcp__wq-brain-http__get_documentations` and `mcp__wq-brain-http__get_documentation_page`
- **API Access**: All MCP tools provide automated API access
- **BRAIN 6-Tips**: Implemented through `mcp__wq-brain-http__create_simulation` calls
...
- **Statistical Analysis**: Use `mcp__wq-brain-http__create_simulation` for data behavior testing
- **Data Quality Assessment**: Use `mcp__wq-brain-http__get_platform_setting_options` for validation
- **Pattern Recognition**: Use `mcp__wq-brain-http__get_operators` for available analysis functions
- **Documentation Management**: Use `mcp__wq-brain-http__get_documentations` for comprehensive knowledge access
- **Community Integration**: Use `search_forum_posts`, `mcp__wq-brain-http__read_forum_post`, and related forum tools (see `brain-forum-browse` skill) for collaborative insights
...
- **`mcp__wq-brain-http__get_datasets`**: Discover and filter available datasets
- **`mcp__wq-brain-http__get_datafields`**: Retrieve field inventory and metadata
- **`mcp__wq-brain-http__create_simulation`**: Execute data analysis simulations
- **`mcp__wq-brain-http__get_platform_setting_options`**: Validate simulation parameters

### **Documentation & Knowledge Tools**
- **`mcp__wq-brain-http__get_documentations`**: Access platform documentation structure
- **`mcp__wq-brain-http__get_documentation_page`**: Read specific documentation content
- **`mcp__wq-brain-http__get_operators`**: Discover available analysis operators
- **`get_documentation_page`**: Access strategy examples and templates

### **Community & Forum Tools**
- **`search_forum_posts`**: Search community discussions
- **`read_forum_post`** / **`mcp__wq-brain-http__read_forum_post`**: Read detailed forum content
- **`mcp__wq-brain-http__get_glossary_terms`**: Access community terminology

### **Competition & Event Tools**
- **`mcp__wq-brain-http__get_events`**: Discover available competitions
- **`mcp__wq-brain-http__get_competition_details`**: Get competition guidelines
- **`mcp__wq-brain-http__get_competition_agreement`**: Access competition rules

### **Best Practices for MCP Tool Usage**
1. **Always authenticate first** using `mcp__wq-brain-http__authenticate`
2. **Validate parameters** using `mcp__wq-brain-http__get_platform_setting_options`
