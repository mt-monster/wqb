...
**Expression**: `ts_mean(datafield, 252) > X`  , for vector data type, the expression should be `ts_mean(vector_operator(datafield), 252) > X`, please note, the vector_operator is the operator that you found via mcp.
**Insight**: Mean of the datafield over ~1 year. Vary the values of X and see the Long Count.

**Watch Out**: `ts_median` is a **ghost operator** on this account — using it CANCELS the entire multi-simulation. Always use `ts_mean`.
