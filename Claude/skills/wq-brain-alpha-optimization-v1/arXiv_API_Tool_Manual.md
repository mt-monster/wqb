...
## 🧠 Concept Extraction & Noise Reduction

### Why `abs:"phrase"` beats plain keywords
A bare query like `"post-earnings-announcement drift"` matches the word *drift* across
all of physics/math, pulling in quantum-diffusion and Euler-Maruyama papers. Wrap the
phrase in `abs:"..."` to match it as a phrase inside the abstract — this returns only
finance-relevant papers (e.g. 3/3 relevant for PEAD).

### `--concepts` (Mode B idea feed)
Every returned abstract is scanned for quantifiable factor concepts (momentum, PEAD,
sentiment, volatility, liquidity, analyst revision, …) and mapped to suggested WQ
operators (`rank`, `ts_zscore`, `ts_rank`, `decay_linear`, `group_rank`, …). This turns
raw papers into implementable alpha ideas instead of raw text.

Add `--llm` to route the abstracts through an OpenAI-compatible chat model for richer,
paper-specific concepts. Configuration precedence (highest first):
1. CLI flags `--llm-model` / `--llm-base`
2. Process env vars `OPENAI_API_KEY` / `OPENAI_BASE_URL` / `OPENAI_MODEL`
3. A sibling `.arxiv_llm.env` file (key=value, gitignored) in the `scripts/` dir
4. Built-in defaults: base `https://api.deepseek.com/v1`, model `deepseek-v4-flash`

Example `.arxiv_llm.env`:
```
OPENAI_API_KEY=sk-...
OPENAI_BASE_URL=https://api.deepseek.com
OPENAI_MODEL=deepseek-v4-flash
```
The base URL is normalized automatically (`https://api.deepseek.com` →
`https://api.deepseek.com/v1/chat/completions`), so either form works.

If the LLM call fails or the key is missing, it automatically falls back to the rule layer.

### Caching & Rate Limiting
- Identical queries (query + `-c` + `-n`) are cached in `.arxiv_cache.json` next to the
  script — re-running costs zero network calls.
- Requests are throttled to >=3s apart to respect arXiv's politeness limits and avoid 503s.
