...
# -*- coding: utf-8 -*-
"""
Pull Skill Folders from a Git repository into a local .qoder/skills directory.
...
- dest_path: Destination folder for skills (default: <cwd>/.qoder/skills (~/.workbuddy/skills 为另一可选安装根))
...
- Scans top-level subfolders for a SKILL.md file (case-insensitive)
- Copies those folders into the destination .qoder/skills directory

Notes:
- Requires Git installed and available in PATH.
- Only validates existence of SKILL.md/skill.md, does not validate YAML content.
...
def main():
    ...
    if not dest:
        dest = os.path.join(os.getcwd(), ".qoder", "skills")
    ensure_dir(dest)
    ...
    for entry in os.listdir(repo_dir):
        ...
        if os.path.isdir(sub):
            # Strict SKILL.md check (copy_skill_folder is case-sensitive on purpose)
