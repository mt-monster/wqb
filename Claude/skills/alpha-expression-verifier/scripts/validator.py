...
# Account-specific ghost operators: syntax looks valid but CANCELS multi-sim.
GHOST_OPERATORS = {
    'ts_median': '幽灵算子 ts_median：会 CANCEL 整个 multi-sim。请改用 ts_mean(..., 252)。',
}
...
supported_functions = {
    ...
    'tail': {'min_args': 1, 'max_args': 4, 'arg_types': ['expression', 'expression', 'expression', 'expression'], 'param_names': ['x', 'lower', 'upper', 'newval'], 'keyword_only_from': 1},  # tail(x, lower=0, upper=0, newval=0)
...
class ExpressionValidator:
    ...
    def validate_function(self, node: ASTNode, is_in_group_arg: bool = False) -> List[str]:
        ...
        if function_name in GHOST_OPERATORS:
            return [GHOST_OPERATORS[function_name]]
