...
# 重点核查数据集：仅作为 EUR 战役(2026-08-05 结论)遗留默认值；其他区域用 --focus-ids 显式指定
REGION_FOCUS = {
    "EUR": [
...
def rank_opportunities(datasets, min_cov=0.85, max_alphas=50, min_fields=10):
    ...
    v3.1 对齐（2026-08-16 走查）：字段数用 usableFieldCount——typed catalog 内
    coverage>=0.85 的字段计数（P3，与 wq-brain-campaign-toolkit score_datasets 同源），
    无 catalog 时回退原始 fieldCount。"""
    def fc(d):
        return d.get("usableFieldCount", d.get("fieldCount", 0))
...
def enrich_usable_fields(datasets, region, catalog_dir):
    """P3 对齐：若战役目录存在 typed catalog（<region小写>_<dataset>_fields.json，
    toolkit 命名规范），用目录内 coverage>=0.85 的字段数作为 usableFieldCount，
    避免 platform 原始 fieldCount 高估可挖广度。无 catalog 的数据集不设该键。"""
    base = Path(catalog_dir)
    if not base.is_dir():
        return 0
    n = 0
    for d in datasets:
        cat = base / f"{region.lower()}_{d.get('id')}_fields.json"
        if not cat.exists():
            continue
        try:
            fields = json.loads(cat.read_text(encoding="utf-8")).get("fields", [])
            usable = sum(1 for f in fields if (f.get("coverage") or 0) >= 0.85)
            if usable:
                d["usableFieldCount"] = usable
                n += 1
        except Exception:
            continue
    return n
...
def slim(d):
    return {k: d.get(k) for k in
            ("id", "name", "category", "coverage", "fieldCount", "usableFieldCount",
...
def main():
    ...
    ap.add_argument("--focus-ids", default=None,
                    help="重点核查数据集 id，逗号分隔；缺省仅 EUR 用历史战役名单")
    ap.add_argument("--catalog-dir", default=None,
                    help="typed catalog 目录（默认 tracking/<REGION>/reference），"
                         "存在则用 catalog 内 cov>=0.85 字段数作 usableFieldCount（P3 对齐）")
    args = ap.parse_args()
    ...
    if args.mode == "mcp":
        try:
            ...
            print(f"[通道] MCP 不可用（{type(e).__name__}: {e}）")
            print(f"[通道] 自动回退直连 {BASE_URL}")
    ...
    if args.dataset_fields:
        ...
        catalog_dir = args.catalog_dir or str(WORKSPACE / "tracking" / region / "reference")
        n_usable = enrich_usable_fields(datasets, region, catalog_dir)
        if n_usable:
            print(f"[P3] {n_usable} 个数据集已用 typed catalog usableFieldCount "
                  f"(cov>=0.85 字段数，目录 {catalog_dir})\n")
        ...
        focus_ids = ([s.strip() for s in args.focus_ids.split(",") if s.strip()]
                     if args.focus_ids else REGION_FOCUS.get(region, []))
        if not focus_ids:
            print("=== 重点数据集核查 ===（无：非 EUR 且未指定 --focus-ids，跳过）\n")
            focus_report = {}
        else:
            print("=== 重点数据集核查 ===")
