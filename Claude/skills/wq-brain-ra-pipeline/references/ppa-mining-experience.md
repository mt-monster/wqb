# PPA 挖掘经验（已并入 wq-brain-ra-pipeline）
...
体检脚本：`wq-brain-ra-pipeline/scripts/dataset_health_check.py`（项目内同源副本 `tools/eur_field_coverage.py`）。
...
### ★ 提交时真实硬门槛（2026-08-11 GBR 战役实测, mcp__wq-brain-http__get_alpha_details 只显示 warning, 提交时才判 FAIL）

| 检查 | 门槛 | 备注 |
|---|---|---|
| LOW_SHARPE | ≥ 1.58 | 严格 |
| LOW_FITNESS | ≥ 1.0 | |
| **LOW_2Y_SHARPE** | **严格 > 1.58** | 1.58 恰好 FAIL! 隐藏门槛, 必须留余量 |
| **CONCENTRATED_WEIGHT** | 必须过 | 最大硬墙, 破解配方见 §6.1 |
| SELF_CORR / PROD_CORR | < 0.7 | **提交时实时查 OS 池**, 与已提交 alpha 高相关直接拒 |
| DATA_DIVERSITY / PPAC / REGULAR_SUBMISSION | PASS | 一般自动过 |

### ★ 提交判定机制（提交统一走 `mcp__wq-brain-http__workflow_submit_alpha`，勿直接信 POST 返回值）

**POST /alphas/{id}/submit → 201 = 受理**（异步受理，~40s 内翻 OS/ACTIVE；旧原生 MCP submit_alpha 只认 200 会误判，已删除，统一走 workflow_submit_alpha）。
**GET /alphas/{id}/submit → 200 = 最终成功 / 403 = 拒绝(带 FAIL 列表) / 404 = 提交记录被清**（此前 201 受理但异步被拒）。
**唯一可靠验证 = alpha 出现在 OS 池（status=ACTIVE）**，不要用 POST 状态或 mcp__wq-brain-http__get_alpha_details 的 checks 判断。
判定脚本模式（底层 BrainApiClient, 绕过 MCP bug）:
```python
r = await client._request('POST', f'{client.base_url}/alphas/{aid}/submit')  # 201
r2 = await client._request('GET', f'{client.base_url}/alphas/{aid}/submit')  # 200 成功 / 403 FAIL
```

### ★ 6.1 CONCENTRATED_WEIGHT 破解配方（2026-08-11 最大发现）

- **症状**：所有 `rank(add(...))` 单输出结构必 CW FAIL（权重集中）。
- **根因**：季度更新字段（如 ep_yield）某些天覆盖不足 → rank 后权重集中在少数股票。
- **解法**：`ts_backfill(字段, 66/125)` 填充缺失 + **线性加权结构**替代 rank(add)：
  ```
  add(multiply(rank(ts_delta(ts_backfill(F, 66), 66)), 0.5),
      multiply(rank(ts_delta(ts_backfill(F, 66), 22)), 0.5))
  ```
  `add(multiply(rank(A), w1), multiply(rank(B), w2))` 通过（含 CW）; `rank(add(A,B))` 必 FAIL。
- **效果**：6 个 backfill 候选 ra_failed:false（含 CW PASS），其中 vRNk56mz 成功提交 ACTIVE（Sh1.80/2Y1.82, predictive_starmine fy1 盈利收益率, GBR/TOP700/D1）。

### ★ 6.2 正交性死结（3 因子组合的隐形约束）

提交时平台**实时查与 OS 池的相关性**（SELF_CORR/PROD_CORR <0.7）。同一数据集同主腿的兄弟表达式互相关 0.82-1.0，第二个起全部被拒——**不要对同主腿变体抱幻想**。要凑 3 个 PPA：
1. 必须**跨数据集/跨主腿**找正交信号（如 starmine fy1 + news + insiders）；
2. 每个新候选提交前先 `mcp__wq-brain-http__compute_mutual_correlation` 验证与已提交池 <0.7；
3. 正交候选常见陷阱：fy2/ARM 主腿虽正交但 2Y 不达标、news 被 INDUSTRY 中和——正交性与达标性往往互斥，需数据源本身有独立强信号。

## 7. 增强版挖掘流程（数据驱动）

0. **平台实时体检（不可跳过）**：`python tools/eur_field_coverage.py --region <R> --delay <D> --universe <U>`
...
- 401 自动重认证：捕获 401 后用凭据重新走 Basic Auth 建立会话再继续请求。
...
- **提交语义（2026-08-11 实测确认，勿再踩坑）**：
  ...
  - 提交前 `mcp__wq-brain-http__get_alpha_details` 的 checks 只显示 warning，**2Y 与 CW 的真实判定在提交时**才暴露；别拿 warning 当 pass。
  - 提交后 alpha 状态短暂保持 UNSUBMITTED（异步 SELF_CORR 等计算中），以 **OS 池出现 ACTIVE** 为最终确认，勿提前下结论。
