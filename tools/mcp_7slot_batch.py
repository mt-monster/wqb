...
提交入口边界（与 tools/submit_batch.py 互补，不合并）：
  ...
  - tools/submit_batch.py 为规范 REST CLI（直接 POST /simulations，payload=[{type,settings,regular}]）。
  - 二者传输层与 payload schema 不同，是回测批量仿真的两条并存客户端；生产提交请用
    world-quant-brain-mcp/tools_submit.py 的 submit_alpha 原语。

用法示例（GBR D1 TOP700，batch=8，7 槽同提）：
...
def main() -> int:
    ...
    ap.add_argument("--max-in-flight", type=int, default=7, help="同时在飞的 multisim 数")
