...
def login():
    # 优先使用环境变量 BRAIN_EMAIL / BRAIN_PASSWORD,否则回退到下方默认凭据
    ...
    for attempt in range(10):
        try:
            ...
            last_err = e
            print(f"login attempt {attempt+1} failed: {type(e).__name__}: {e}", flush=True)
...
def get_datafields(
    ...
    if len(search) == 0:
        ...
        while True:
            ...
            if "retry-after" in count_resp.headers:
                time.sleep(float(count_resp.headers["Retry-After"]))
                continue
            try:
    ...
    for x in range(0, count, 50):
        while True:
            ...
            if "retry-after" in datafields.headers:
                time.sleep(float(datafields.headers["Retry-After"]))
                continue
            try:
...
def get_alphas(start_date, end_date, sharpe_th, fitness_th, region, alpha_num, usage, year=2026):
    ...
    for i in range(0, alpha_num, 100):
        ...
        url_e = "https://api.worldquantbrain.com/users/self/alphas?limit=100&offset=%d"%(i) \
                + "&status=UNSUBMITTED%1FIS_FAIL&dateCreated%3E=%d-"%(year) + start_date  \
                + "T00:00:00-04:00&dateCreated%3C%d-"%(year) + end_date \
...
def get_first_order(vec_fields, ops_set, region="usa"):
    alpha_set = []
    #for field in fields:
...
def group_factory(op, field, region):
    ...
    # MEA 专用分组:无 fnd28(bps_group 不可用),改用 sta2_top400 统计行业聚类 + 通用分组
    mea_group_2 = ['sta2_top400_fact1_c10', 'sta2_top400_fact1_c5', 'sta2_top400_fact2_c10',
                   'sta2_top400_fact2_c5', 'sta2_top400_fact3_c10', 'sta2_top400_fact3_c5',
                   'sta2_top400_fact4_c10', 'sta2_top400_fact4_c5', 'sta2_top400_fact4_c2']

    if region == "mea":
        groups = ["market", "sector", "industry", "subindustry", cap_group, sector_cap_group, vol_group]
        groups += mea_group_2
