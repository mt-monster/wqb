...
import os
"""
wave17 ASI analyst46_anl39 CNN enhance runner.
Input: asi_wave17_cnn_items.json [{code, note}],
5 multisim concurrent fill (batch_size=8), poll to TERMINAL, fetch IS metrics,
write JSON/CSV. 复用五槽填槽模式（wqb-concurrency §8）。
"""
import csv
import json
import sys
...
DATASET = "analyst46_anl39"
INPUT = "tracking/IND/candidates/ind_wave30_items.json"
SLOTS = 5
BATCH_SIZE = 8
POLL_INTERVAL = 15
STALL_MINUTES = 8
...
def submit_batch(api, settings, batch_idx, batch):
    ...
    for it in batch:
        ...
        if it.get("universe") is not None:
            s["universe"] = it["universe"]
...
def main():
    ctx = CampaignContext("tracking/IND")
    ...
    out_dir = Path("tracking/IND/results")
    out_dir.mkdir(parents=True, exist_ok=True)
    out_json = out_dir / "wave30_results.json"
    out_csv = out_dir / "wave30_results.csv"
