# 波5 闸门评估: 按表达式匹配 (一次性脚本)
import json

p = r'C:\Users\MENGTAO\.qoder-cn\cache\projects\wqb-a40170ec\agent-tools\task-080\fd12c920.txt'
d = json.load(open(p, encoding='utf-8'))

files = [
    r'd:\coding\traeCN_project\wqb\tracking\USA\runs\usa_news_batch_cc.txt',
    r'd:\coding\traeCN_project\wqb\tracking\USA\runs\usa_news_batch_dd.txt',
    r'd:\coding\traeCN_project\wqb\tracking\USA\runs\usa_news_batch_ee.txt',
    r'd:\coding\traeCN_project\wqb\tracking\USA\runs\usa_news_batch_ff.txt',
    r'd:\coding\traeCN_project\wqb\tracking\USA\runs\usa_news_batch_gg.txt',
    r'd:\coding\traeCN_project\wqb\tracking\USA\runs\usa_news_batch_hh.txt',
    r'd:\coding\traeCN_project\wqb\tracking\USA\runs\usa_news_batch_ii.txt',
]
want = set()
for f in files:
    try:
...
for a in d['results']:
    ...
    rows.append((sh, a['id'], a['settings']['decay'], a['settings']['neutralization'][:3],
                 fit, tv, round(mg * 1e4, 1), y2, rn, su, int(ra.get('failed_ra_count', 0)), ok, code[:68]))
