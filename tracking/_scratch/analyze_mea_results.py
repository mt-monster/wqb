...
with open(r'C:\Users\MENGTAO\.qoder-cn\cache\projects\wqb-a40170ec\agent-tools\task-d8b\9968f2f4.txt') as f:
    data = json.load(f)

print(f'Total MEA alphas: {data["count"]}')
print('\n筛选达标 alpha (sharpe>1.58, fitness>1, turnover 5%-30%):')
print('='*80)
...
for alpha in data['results']:
    ...
    if (m['sharpe'] > 1.58 and m['fitness'] > 1 and 
        ...
        print(f"  Code: {alpha['code'][:80]}...")
        print()

print(f'\n达标 alpha 数量: {len(qualified)}/70')

# 也显示 sharpe>1.5 的 alpha（接近达标）
...
print(f'\n接近达标 alpha 数量: {len(near_qualified)}/70')
