...
# -*- coding: utf-8 -*-
"""
wave6 EUR FCF x hetero-component dilution mix probe runner.
Input: eur_wave6_fcfmix_items.json [{code, dataset, tag}],
...
INPUT = "tracking/EUR/candidates/eur_wave6_fcfmix_items.json"
SLOTS = 5
BATCH_SIZE = 8
POLL_INTERVAL = 15
STALL_MINUTES = 8
...
def passes_user_thresholds(r, t):
    ...
    if not (rn["sharpe"] is not None and rn["sharpe"] > t.get("rn_sharpe_min", 1.0)):
        return False
    if not ((rn["fitness"] or 0) > t.get("rn_fitness_min", 0.7)):
        return False
    return True
...
def main():
    ...
    out_json = out_dir / "wave6_fcfmix_probe_results.json"
    out_csv = out_dir / "wave6_fcfmix_probe_results.csv"
    ...
    for r in results[:12]:
        print(
            f"{r['id']} d{r.get('decay')} t{r.get('truncation')} "
            f"sh={r.get('sharpe') or 0:.2f} fit={r.get('fitness') or 0:.2f} "
