...
import os
"""
wave20 EUR model36 credit-risk LOW-PRIORITY probe runner.
Input: eur_wave20_m36low_items.json [{code, note}],
...
降级探针：信用风险在 EUR 三次实证弱（wave1/6/8），但结构重构（滞后/中性化/交互）未验证。
本批 16 探针测试：ts_delay 5/10 日、group_neut country、multiply 20日动量交互。
若仍全崩，判死并记录台账："信用风险在 EUR 经结构重构验证仍弱"。
"""
import csv
import json
import sys
...
DATASET = "model36"
INPUT = "tracking/EUR/candidates/eur_wave20_m36low_items.json"
SLOTS = 5
BATCH_SIZE = 8
POLL_INTERVAL = 15
STALL_MINUTES = 8
...
def main():
    ...
    out_json = out_dir / "wave20_m36low_results.json"
    out_csv = out_dir / "wave20_m36low_results.csv"
