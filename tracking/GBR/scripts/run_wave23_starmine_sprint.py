...
import os
"""
wave23 predictive_starmine sprint runner (per-item decay overrides).
Input: gbr_wave23_starmine_sprint_items.json [{code, decay}], 3 batches x 8 items,
...
INPUT = "tracking/GBR/candidates/gbr_wave23_starmine_sprint_items.json"
SLOTS = 5
BATCH_SIZE = 8
POLL_INTERVAL = 15
STALL_MINUTES = 8
...
def save_progress(out_json, out_csv, results, candidates):
    ...
    fieldnames = [
        "id", "dataset", "code", "decay", "sharpe", "fitness", "two_year_sharpe",
...
def submit_batch(api, settings, batch_idx, batch):
    ...
    for it in batch:
        ...
        s["decay"] = it.get("decay", s.get("decay", 4))
...
def main():
    ...
    out_json = out_dir / "wave23_starmine_sprint_results.json"
    out_csv = out_dir / "wave23_starmine_sprint_results.csv"
    ...
    while inflight:
        ...
        for msid in list(inflight.keys()):
                ...
                if progress != prev_prog:
                    ...
                    for it in rec["batch"]:
                        results.append({
                            ...
                            "decay": it.get("decay"), "multisim": msid,
                            "batch_idx": rec["batch_idx"], "error": "STALLED",
                        })
                else:
            ...
            if msid in last_seen:
                del last_seen[msid]
            decay_map = {it["code"]: it.get("decay") for it in rec["batch"]}
            rows = fetch_rows_for_multisim(api, fetcher, msid)
            for r in rows:
                r["dataset"] = DATASET
                r["multisim"] = msid
                r["batch_idx"] = rec["batch_idx"]
                r["decay"] = decay_map.get(r.get("code"))
                results.append(r)
    ...
    for r in results[:12]:
        print(
            f"{r['id']} d{r.get('decay')} sh={r.get('sharpe') or 0:.2f} "
            ...
            f"rn={r.get('rn_sharpe') or 0:.2f} {r.get('code', '')[:45]}"
        )
