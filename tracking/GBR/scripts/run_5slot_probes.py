...
# -*- coding: utf-8 -*-
"""
5-slot concurrent multisim runner for GBR Stage-A probes.
Reads per-dataset stageA expression files, keeps up to 5 multisim in flight,
...
# ---------------------------------------------------------------------------
DATASETS = {
    "other455": "tracking/GBR/candidates/probe_other455_stageA_exprs.json",
    "pattern_scores": "tracking/GBR/candidates/probe_pattern_scores_stageA_exprs.json",
    "analyst_earnings_ibes": "tracking/GBR/candidates/probe_analyst_earnings_ibes_stageA_exprs.json",
    "news104": "tracking/GBR/candidates/probe_news104_stageA_exprs.json",
    "dl_riskfree_returns": "tracking/GBR/candidates/probe_dl_riskfree_returns_stageA_exprs.json",
}
SLOTS = 5
BATCH_SIZE = 8
POLL_INTERVAL = 15
...
def save_progress(out_json, out_csv, results, candidates):
    """Incremental save after each multisim completes."""
...
def fetch_rows_for_multisim(api, fetcher, msid):
    """Return list of metric rows for a completed multisim."""
...
def passes_user_thresholds(r, t):
    """Check review thresholds plus RN view thresholds requested by user."""
    ...
    if not ((rn["fitness"] or 0) > 0.7):
        return False
    # margin > 5bp already enforced above (margin_bp > 50)
    return True
...
def main():
    ...
    inflight = {}  # msid -> {"ds", "batch_idx", "batch"}
    results = []
    candidates = []

    out_dir = Path("tracking/GBR/results")
    out_dir.mkdir(parents=True, exist_ok=True)
    out_json = out_dir / "5slot_probe_results.json"
    out_csv = out_dir / "5slot_probe_results.csv"
    ...
    last_seen = {}  # msid -> (progress, last_change_ts)
        ...
        for msid in list(inflight.keys()):
            ...
            if status not in TERMINAL:
                ...
                if progress != prev_prog:
                    ...
                    # record a failure row for each expression so we don't lose track
