# -*- coding: utf-8 -*-
"""gate.py - KOR 统一提交前闸门（M4/M7/M9/M10/M11）。

合并 batch_validate_kor.py（语法）+ kor_preflight_check.py（白名单/类型/禁用）为单步：
  闸1 语法：直接 import ExpressionValidator（不再每表达式一次子进程；M9）
  闸2 字段白名单：--dataset 自动派生 reference/kor_<ds>_fields.json（typed catalog 优先）
      或 reference/kor_<ds>_field_whitelist.json（legacy 兼容；M10）
  闸3 类型：catalog 有字段级 type 时数据驱动判定 VECTOR 包裹（不再正则猜；M4），
      legacy 白名单退回 strip 启发式；MATRIX 禁 vec_*
  闸4 平台不可访问算子（ts_min/ts_max）+ quantile 仅 1 参 + 白名单 banned_patterns
  闸5 毒模式：嵌套三腿 add 等（reference/kor_generation_constraints.json，record_poison 实证；M11）
  结果缓存 cache/gate_cache.json（同 expr+dataset 幂等跳过）

用法:
  python gate.py --dataset model219 --file candidates/xxx_exprs.json
  python gate.py --dataset model219 --expr "rank(close)"
...
# ---- verifier：import 直调（M9），路径搜索 .workbuddy 优先 ----
_VALIDATOR_DIRS = [
...
DRIVER_ARGS = {"gaussian", "uniform", "cauchy"}  # quantile(x, driver=...) 的驱动参数标识符

GATE_CACHE = os.path.join(ROOT, "cache", "gate_cache.json")
os.makedirs(os.path.dirname(GATE_CACHE), exist_ok=True)


def load_cache():
    try:
        return json.load(open(GATE_CACHE, encoding="utf-8"))
    except Exception:
        return {}
...
def load_whitelist(dataset):
    """typed catalog 优先，legacy 白名单兜底。返回 (verified_ids, data_type, field_types, banned)。"""
    cat = os.path.join(ROOT, "reference", f"kor_{dataset}_fields.json")
    ...
    wl = os.path.join(ROOT, "reference", f"kor_{dataset}_field_whitelist.json")
    if os.path.exists(wl):
        ...
        fts = {f["id"]: f.get("type") for f in d.get("fields", [])}
        return set(fts), d.get("data_type", "MATRIX"), fts, d.get("banned_patterns", [])
    raise FileNotFoundError(f"无白名单/catalog：先跑 scan_fields.py --dataset {dataset}")


def fn_spans(expr):
    """解析全部函数调用区间 -> [(start, end, fn_name)]（用于数据驱动 VECTOR 包裹判定）。"""
    spans, stack = [], []
    i = 0
    while i < len(expr):
...
def naked_vector_fields(expr, fields, field_types):
    """字段级 type==VECTOR 且未被 vec_* 直接包裹的字段列表（数据驱动，M4）。"""
    spans = fn_spans(expr)
    naked = []
    for f in fields:
...
def legacy_strip_naked(expr, fields):
    """legacy 白名单无字段级 type 时的 strip 启发式（兼容 kor_preflight_check 3e）。"""
    stripped = expr
...
def check_one(expr, wl, dataset, poison_patterns):
    ...
    # 注意：ts_min/ts_max 不在 KNOWN_OPS 中，必须对 idents 判定（原 preflight 对 ops_used 判定是死代码）
