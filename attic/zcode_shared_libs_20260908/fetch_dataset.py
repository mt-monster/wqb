...
def find_config(start_dir):
    """Walk up from start_dir to find the nearest config.json (unified format aware).
    Also checks current working directory first (wrapper runpy changes __file__)."""
    d = Path(start_dir)
    # CWD first: wrapper runpy makes __file__ point at shared_libs, but user runs from skill root
    cwd = Path.cwd()
    for base in (cwd, d):
        p = base
        for _ in range(10):
            for cand in (p / "config.json", p / "configs" / "config.json"):
                if cand.exists():
                    return cand
            if p.parent == p:
                break
...
def extract_credentials(config):
    """Extract (email, password) from any config format:
    unified {credentials:{email,password}} / BRAIN_CREDENTIALS / flat email / brain_email
    """
    ...
    if isinstance(config.get("BRAIN_CREDENTIALS"), dict):
        c = config["BRAIN_CREDENTIALS"]
        return c.get("email", ""), c.get("password", "")
    if config.get("brain_email"):
        return config.get("brain_email", ""), config.get("brain_password", "")
...
def main():
    ...
    config_path = find_config(workspace_dir)
    data_dir = workspace_dir / "data"

    # Ensure data directory exists
    data_dir.mkdir(parents=True, exist_ok=True)

    # Load configuration (env override -> config)
    if not config_path:
        print("Error: Config file not found. Run from skill root or set BRAIN_EMAIL/BRAIN_PASSWORD env.")
        sys.exit(1)
