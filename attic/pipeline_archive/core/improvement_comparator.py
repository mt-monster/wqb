# -*- coding: utf-8 -*-
"""pipeline/core/improvement_comparator.py - 通用改进对比分析器（区域无关）
...
用法:
  from pipeline.core.improvement_comparator import ImprovementComparator
  
  comparator = ImprovementComparator(
      historical_ledger="tracking/KOR/kor_d1_campaign_state.json",
...
class ImprovementComparator:
    ...
    def __init__(self, historical_ledger: str, monitor_dir: str):
        """初始化
        
        Args:
            historical_ledger: 历史台账文件路径（改进前数据）
            monitor_dir: 监控数据目录（改进后数据）
        """
        self.historical_ledger = historical_ledger
        self.monitor_dir = monitor_dir
    
    def _load_historical_waves(self) -> List[Dict]:
        """加载改进前的历史波次数据"""
        if not os.path.exists(self.historical_ledger):
            return []
        
        with open(self.historical_ledger, encoding="utf-8-sig") as f:
            state = json.load(f)
    ...
    def _load_monitoring_waves(self) -> List[Dict]:
        """加载改进后的监控波次数据"""
        monitoring = []
    ...
    def _calculate_metrics(self, waves: List[Dict], label: str) -> Optional[Dict]:
        """计算效率指标"""
        if not waves:
            return None
    ...
    def generate_comparison_report(self) -> Optional[Dict]:
        """生成对比报告"""
        # 加载数据
        historical = self._load_historical_waves()
        monitoring = self._load_monitoring_waves()
        
        # 计算指标
        before = self._calculate_metrics(historical, "改进前")
        after = self._calculate_metrics(monitoring, "改进后")
        ...
        # 保存报告
        report_path = os.path.join(self.monitor_dir, f"improvement_comparison_{datetime.date.today().isoformat()}.json")
        with open(report_path, "w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=1)
        
        # 打印报告
        self._print_comparison(report, report_path)
        
        return report
    
    def _print_comparison(self, report: Dict, report_path: str):
        """打印对比报告"""
