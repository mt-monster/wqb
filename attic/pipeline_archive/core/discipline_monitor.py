# -*- coding: utf-8 -*-
"""pipeline/core/discipline_monitor.py - 通用纪律监控器（区域无关）

监控实施"有纪律地切换"流程后 N 轮回测的表现，验证改进是否有益。
...
用法:
  from pipeline.core.discipline_monitor import DisciplineMonitor
  
  monitor = DisciplineMonitor(monitor_dir="tracking/KOR/monitoring")
  monitor.start_monitoring("17A", "other455")
  monitor.record_batch("17A", "batch_0", 8, 8, 2)
  monitor.complete_monitoring("17A")
  report = monitor.generate_report(waves=10)
"""
import collections
import datetime
import glob
...
class DisciplineMonitor:
    ...
    def _get_monitor_file(self, wave: str) -> str:
        """获取监控数据文件路径"""
        return os.path.join(self.monitor_dir, f"discipline_monitor_{wave}.json")
    
    def _load_monitor_data(self, wave: str) -> Dict:
        """加载监控数据"""
        path = self._get_monitor_file(wave)
    ...
    def _save_monitor_data(self, data: Dict):
        """保存监控数据"""
        path = self._get_monitor_file(data["wave"])
        tmp = path + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=1)
        os.replace(tmp, path)
    
    def start_monitoring(self, wave: str, dataset: str):
        """开始监控一个波次"""
        data = self._load_monitor_data(wave)
        data["started_at"] = datetime.datetime.now().isoformat(timespec="seconds")
        data["dataset"] = dataset
        self._save_monitor_data(data)
        print(f"[monitor] 开始监控 wave={wave} dataset={dataset}")
    
    def record_batch(self, wave: str, batch_id: str, exprs: int, complete: int, candidates: int, prod_stats: Optional[Dict] = None):
        """记录一个批次的回测结果"""
        data = self._load_monitor_data(wave)
        ...
        data["efficiency_metrics"]["total_quota_used"] += exprs
        
        self._save_monitor_data(data)
        print(f"[monitor] 记录批次 wave={wave} batch={batch_id} exprs={exprs} complete={complete} candidates={candidates}")
    
    def record_discipline_decision(self, wave: str, decision_type: str, details: Dict):
        """记录纪律决策"""
        data = self._load_monitor_data(wave)
    ...
    def complete_monitoring(self, wave: str):
        """完成监控"""
        data = self._load_monitor_data(wave)
    ...
    def generate_report(self, waves: int = 10) -> Dict:
        """生成实证报告"""
        # 收集所有监控数据
        all_data = []
        for f in sorted(glob.glob(os.path.join(self.monitor_dir, "discipline_monitor_*.json"))):
            try:
        ...
        # 保存报告
        report_path = os.path.join(self.monitor_dir, f"discipline_report_{datetime.date.today().isoformat()}.json")
        with open(report_path, "w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=1)
        
        # 打印报告
        self._print_report(summary, report_path)
        
        return report
    
    def _generate_conclusion(self, summary: Dict) -> List[str]:
        """生成结论"""
        conclusions = []
    ...
    def _print_report(self, summary: Dict, report_path: str):
        """打印报告"""
        print("\n" + "=" * 60)
        print("战役纪律改进实证报告")
        print("=" * 60)
        ...
        print("\n结论:")
        for c in self._generate_conclusion(summary):
            print(f"  - {c}")
        print("=" * 60)
